...
public class AuthService {
    ...
    @Autowired
    public AuthService(EmailSenderService emailService,
        ...
        if (resetCode != null && person != null) {
            ...
            if (entity != null) {
                ...
                if (timeEntity.isBefore(LocalDateTime.now())) {
                    ...
                    codeRepository.save(new ResetCodeEntity(person,
                            resetCode, LocalDateTime.now().plusMinutes(codeValidityTime)));
                }
            ...
            sendEmailAuthMessage(currentResetCode, person.getEmail(),
                    secretCodeEmailMessage, secretCodeEmailHeader);
        }
    }
    ...
    public void changePassword(ResetPasswordDto passwordDto) {
        PersonEntity currentPerson = personService.loadUserByEmail(passwordDto.email());
        if (currentPerson != null) {
            currentPerson.setPassword(encodePass(passwordDto.newPassword()));
    ...
    public PersonEntity registerNewUserByEmail(RegisterByEmailDto byEmailDto) {
        PersonEntity person = personService.loadUserByEmail(byEmailDto.getEmail());
        String password = encodePass(byEmailDto.getPassword());
