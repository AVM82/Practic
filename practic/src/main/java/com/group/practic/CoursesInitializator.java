...
public class CoursesInitializator {
    ...
    @PostConstruct
    void initialize() {
        File[] files = new File(".").listFiles(f -> f.isFile()
                && f.getName().endsWith(COURSE_MASK));
        for (File file : files) {
        ...
        if (PropertyLoader.isComprehendedString(filename)) {
            ...
            if (propCourse.initialized) {
                Long courseId = courseService.create(getAuthorSet(propCourse),
                        propCourse.getProperty(TYPE_KEY, ""),
                        propCourse.getProperty(NAME_KEY, ""),
                        propCourse.getProperty(PURPOSE_KEY, ""),
                        propCourse.getProperty(DESCRIPTION_KEY, ""),
